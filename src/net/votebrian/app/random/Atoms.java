...
public class Atoms {
    ...
    public Atoms(Context context, GL10 gl) {
        mCtx    = context;
        mGl     = gl;
    ...
    public void updateWallpaper(float screenWidth, float screenHeight) {
        mScreenWidth = 0.95f * screenWidth;
    ...
    private void initializeWallpaper() {
        ...
        for(row = 0; row < mNumRows; row++) {
            for(col = 0; col < mNumCols; col++) {
                ...
                mDots[index].setPosition( (-1f*(mScreenWidth-mAtomRadius)) + (float)(1.1 * mAtomRadius * col), (mScreenHeight-mAtomRadius)  - (float)(1.1 * mAtomRadius * row), 0f);
